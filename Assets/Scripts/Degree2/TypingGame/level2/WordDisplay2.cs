...
public class WordDisplay2 : MonoBehaviour
{
    ...
    {
        int count = FindAnyObjectByType<WordManager2>().count;
        if ( count == 5){
           FindAnyObjectByType<WordManager2>().openSecurity.SetActive(true);

        }

         //firewall, network, policy, risk, phishing
        if (check == "eavesdropping")
        {
        ...
        FindAnyObjectByType<WordManager2>().count=count;
    }
